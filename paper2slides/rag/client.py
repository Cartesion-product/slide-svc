...
class RAGClient:
    ...
    def _create_embedding_func(self) -> EmbeddingFunc:
        ...
        # Build embedding-specific API kwargs
        # Use separate embedding API settings if configured (e.g., Ollama)
        if api.embedding_base_url:
            embedding_kwargs = {
                "api_key": api.embedding_api_key or "ollama",
                "base_url": api.embedding_base_url,
            }
        else:
            # Default: use same API settings as LLM
            embedding_kwargs = self._get_api_kwargs()
