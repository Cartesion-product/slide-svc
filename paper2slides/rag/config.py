...
DEFAULT_OUTPUT_DIR = PROJECT_ROOT / "rag" / "outputs"
...
class APIConfig:
    ...
    # Separate embedding API settings (for Ollama or other local embedding services)
    embedding_base_url: Optional[str] = field(
        default_factory=lambda: os.getenv("EMBEDDING_BASE_URL", None)
    )
    """Optional. Separate base URL for embedding API (e.g., Ollama: http://localhost:11434/v1)."""
    
    embedding_api_key: Optional[str] = field(
        default_factory=lambda: os.getenv("EMBEDDING_API_KEY", "ollama")
    )
    """API key for embedding service. Ollama uses 'ollama' as placeholder."""
