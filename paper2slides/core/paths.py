...
def get_config_name(config: Dict) -> str:
    """Generate config directory name: {outputs}_{style}_{param}."""
...
def get_config_dir(base_dir: Path, config: Dict) -> Path:
    """Get config-specific directory for plan and outputs."""
...
def get_output_dir(config_dir: Path) -> Path:
    """Get outputs directory with timestamp to preserve history."""
