...
async def chat(
    ...
    style: str = Form("academic"),  # 'academic', 'doraemon', or custom description
    ...
    """
    try:
        print(f"===================> params message info: {message}")
...
@app.post("/api/chat_with_paths", response_model=ChatResponse)
async def chat_with_paths(
    background_tasks: BackgroundTasks,
    message: str = Form(""),
    content: str = Form("paper"),  # 'paper' or 'general'
    output_type: str = Form("slides"),  # 'slides' or 'poster'
    ...
    fast_mode: Optional[str] = Form(None),  # 'true' or 'false' - fast mode for paper content
    session_id: Optional[str] = Form(None),  # Existing session ID to reuse files
    file_paths: str = Form(""),  # Comma-separated file paths
):
    """
    Chat endpoint that receives file paths instead of uploaded files
    ...
    Args:
        ...
        fast_mode: 'true' or 'false' - fast mode for paper content (no RAG indexing)
        session_id: Optional existing session ID to reuse files (for regeneration)
        file_paths: Comma-separated list of file paths on the server
    ...
    try:
        ...
        # Process file paths
        file_path_list = [path.strip() for path in file_paths.split(",") if path.strip()] if file_paths else []
        ...
        # Check if reusing existing session
        reusing_session = False
        if session_id and not validated_files:
            # Reuse existing session
            session_dir = UPLOAD_DIR / session_id
            if session_dir.exists():
                reusing_session = True
            ...
            # Copy files from provided paths to session directory
            saved_files = []
        ...
        # Load files for reusing session or use copied files
        saved_files = []
        if reusing_session:
            # Load existing files from session directory
            for file_path in session_dir.iterdir():
            ...
            print(f"Loaded {len(saved_files)} existing file(s) from session")
        # If not reusing session, files have already been copied and saved_files is already populated
        ...
        # Log received request
        print(f"\n{'='*60}")
        print(f"New Request with File Paths (Session: {session_id[:8]})")
        print(f"Files: {len(saved_files)} file(s)")
        for f in saved_files:
            print(f"  - {f['filename']} ({f['size']} bytes)")
        print(f"Config: {output_type} | {style} | {content}")
        ...
        print(f"Error processing request with file paths: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error processing request: {str(e)}")
...
async def generate_slides_with_pipeline(
    ...
    Returns:
        Dictionary with slides info and outputs paths
    """
    parser_enabled = os.getenv("PARSER_ENABLED").lower() == "true"

    # Find PDF files (support multiple PDFs in one session)
    pdf_files = [f for f in files if f['filename'].lower().endswith('.pdf')]
    if not pdf_files:
        if not parser_enabled:
            md_files = [f for f in files if f['filename'].lower().endswith('.md')]
            pdf_files.extend(md_files)
            if not pdf_files:
                raise ValueError("No PDF or MD file found in uploaded files")
        else:
...
async def get_status(session_id: str):
    ...
    try:
        # Find the outputs directory for this session
        session_dir = UPLOAD_DIR / session_id
...
async def get_result(session_id: str):
    ...
    try:
        ...
        if hasattr(app.state, 'results') and session_id in app.state.results:
            ...
            # Prepare response with outputs files
            output_files = result.get("output_files", [])
            
            # Find PDF file in outputs
            ...
            # Try to get outputs type from state
            from paper2slides.core.paths import get_base_dir
            import json
            ...
            response_data = {
                "session_id": session_id,
                "output_dir": result.get("output_dir"),  # 本地输出目录路径
                "output_files": output_files,  # 完整的文件列表（包含本地路径）
                "slides": [
                    {
                        "title": f"Slide {i+1}",
